...
| 命中率高只是「少收钱地多算」,不影响实算吞吐 | **实算吞吐本身也涨 1.93 倍。** prefill 工作量少 78%,腾出的 batch 预算给了 decode,输出吞吐正好翻倍(226.9 对 113.9 tok/s)。每卡计费产能热态是冷态的 **8.8 倍** = 4.55(命中乘数)× 1.93(实算吞吐)(§3.4.2) |
| 并发越高会摊薄前缀缓存命中率 | **单副本内完全不摊薄。** 并发 1→64,命中率恒定 78.0~78.1%,全程 0 抢占。摊薄只发生在**跨副本**(无亲和时的 1/N)。同副本内并发与命中率正交(§3.4.2) |
...
13. **一个端点上只能有一个压测客户端。** 实例启动失败后我在同一端口重开,
...
| 并发 | 命中率 | 实算输入 | **计费输入** | 输出 | 热 TTFT p50 | 热 TTFT p90 | 全 TTFT p90 | TPOT | 抢占 |
|---|---|---|---|---|---|---|---|---|---|
| 1 | 78.0% | 7,823 | 35,614 | 81.3 | 0.56s | 0.58s | 1.97s | 10.0ms | 0 |
| 2 | 77.9% | 9,565 | 43,222 | 99.6 | 0.56s | 0.61s | 2.18s | 17.1ms | 0 |
| 4 | 78.0% | 10,175 | 46,341 | 105.8 | 0.64s | 0.77s | 2.46s | 33.2ms | 0 |
| 8 | 78.1% | 13,055 | 59,557 | 135.9 | 0.84s | 1.20s | 3.77s | 49.1ms | 0 |
| 16 | 78.1% | 17,250 | 78,725 | 179.5 | 0.98s | 1.49s | 4.49s | 73.3ms | 0 |
| **32** | 78.1% | **19,759** | **90,039** | **205.5** | **0.96s** | **1.72s** | 15.36s | 110.3ms | 0 |
| 48 | 78.0% | 21,400 | 97,444 | 222.5 | **14.57s** | 19.04s | 47.23s | 112.7ms | 0 |
| 64 | 78.0% | 21,804 | 99,333 | 226.9 | 14.27s | 19.34s | 49.09s | 113.6ms | 0 |

计费输入 = 实算输入 ÷ (1 − 命中率),即 §1.1 的乘数关系。
...
```
每副本  实算输入 19,759 tok/s   计费输入 90,039 tok/s
每卡    实算      2,470 tok/s   计费     11,255 tok/s      (8 卡/副本)
4 副本  计费 360,156 tok/s
```

对比 §3.4.1 同一档(conc=32,0% 命中)的每卡 1,283 计费 tok/s,**热态是 8.8 倍**。
这 8.8 倍要拆成两个独立的因子,不要混成一个:

```
4.55×  命中率的计费乘数    1/(1−0.781)
1.93×  实算吞吐本身也涨了   19,759 / 10,261
```

第二个因子容易被忽略:**命中不只是"少收钱地多算",它让同一份 batch 预算
(`--max-num-seqs=32` × `--max-num-batched-tokens=2048`)服务更多请求** ——
prefill 工作量少了 78%,腾出的预算给了 decode。所以输出吞吐也正好翻倍
(226.9 对 113.9 tok/s),热态 TPOT 约为冷态的一半(10.0/17.1/33.2/49.1/73.3ms
对 10.2/24.2/43.4/83.8/143.1ms)。
...
1. **混合形状的容量扫描**。§3.4.2 给出了 23k 形状下的可卖容量
   (每卡计费 11,255 tok/s,运行点 conc=32),§3.4 给出了 170k 形状下的
   2441.6 tokens/s/卡 —— **差一个数量级**。按 §1.2 的真实分布(中位 39.5k)
   混合长度重跑,才是能拿去定价的数。**这是现在最便宜的高价值项。**
2. **预热**(§7.3)。对 1M 上下文不是优化而是前提条件:首轮 200s vs 命中 1~2s
   (§3.9.1)。一次预热同时暖推理侧和审核侧的缓存。
   **前置问题:先去 new-api 日志确认首轮请求是不是「文档先到、问题后到」。**
3. **准入控制**(§3.4):按活跃上下文总量限流到 1.4M token/实例。
