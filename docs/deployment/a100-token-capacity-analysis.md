...
14. **重尾分布上,小样本的均值毫无意义。** 混合形状扫描第一版每档随机抽 6~12 个
    ...
    这种尾巴下随机抽样需要几百个样本才收敛。
15. **一个端点上只能有一个压测客户端。** 实例启动失败后我在同一端口重开,
...
6. **缓存读的计价策略**:跟不跟供应商的 3.1% 折扣 —— 86% vs 57% 毛利的分界,
   需业务决策。
7. **解码速率的差距**(§2.4):我们 ~50 tok/s 对百度 124.6 tok/s,差 2.5 倍,
   直接决定能不能通用平替百度(平衡点约 35 个输出 token)。
   **注意不是简单「缩上下文换解码」** —— 同事那套 400k 配置解码 37.9 tok/s 比我们
   还慢。要单独做拓扑/投机解码实验。
8. **审核与推理并发化**(§7.2.3),消掉首轮那 2.76 秒。要动核心计费链路,
   需业务决策。审核目前**已关闭**(2026-09-15 起,为压测定位而关,尚未恢复)。
9. **实例健康巡检**:§7.2.2 那个盲区(状态 running 但端口不听)会让网关持续往死
   实例分流。对比压测时撞到过一次,约 1/4 请求 503 持续十来分钟。
   判据是直连各实例口拉 `/metrics`,不能只看 gpustack 的状态字段。
10. 未完成:LMCache 后端 vs native 的对比(尝试时被 Ray 陈旧 actor 句柄打断,
   重启 Ray 后未重跑)。
