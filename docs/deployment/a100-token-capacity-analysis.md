...
**顺带纠正一条常见误解:PD 分离不是被带宽卡死的。** §7.4 的生死线是约 1 GB/s,
而 §7.5 实测 RoCE 是 50 GB/s(四张齐上)/ 12.5 GB/s(单张),高出一到两个数量级。
真正卡住的是软件状态:`nixl` 是**我们自己故意没装**的(§7.8:「`Requires-Dist:
torch`,有覆盖 base 的风险;且用途是 PD 分离而非共享缓存」—— 当时的前提是
"我们要的是共享缓存",需求已经变了),`mooncake` 写路径每个 key 都失败(§7.7.1)。
**所以 PD 分离是个没做的工程项,不是做不到的事;但它也不该是解决阻塞的路径。**
