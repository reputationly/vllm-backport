...
| 队头阻塞是长上下文推理的固有代价 | **火山完全没有(0.9×)**,而且 300k prefill 只用 13.5s(22,200 tok/s)。我们 7.3×、同事 2.0×。这是我们的工程差距,不是物理限制(§3.4.4) |
...
| | A 小请求 p50 | B 小请求 p50 | **p50 恶化** | p90 恶化 | 300k 大请求自身 |
|---|---|---|---|---|---|
| 我们(1M,直连单副本) | 9.62s | 70.56s | **7.3×** | 5.5× | 32.0s / 63.6s |
| 同事(400k,经网关 4 副本) | 7.09s | 14.47s | 2.0× | 5.5× | 54.6s / 64.1s |
| **火山** | 2.14s | **1.90s** | **0.9×(无阻塞)** | **0.9×** | **13.8s / 13.4s** |

**火山完全没有这个问题**,小请求在两个 300k 并存时甚至略快(噪声内),而且它
300k 的 prefill 只用 13.5s ≈ 22,200 tok/s(我们 256k 时是 9,950)。基本只能是
**PD 分离**(prefill 与 decode 在不同池子)加足够大的机队。
...
1. **按 prompt 大小分流**(§7.1 已提出,§3.4.3 / §3.4.4 给了定量依据)。
   ...
   (火山 300k 只要 13.5s,我们 32~64s,这一类它本来就更强)。
