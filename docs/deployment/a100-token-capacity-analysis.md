...
### 3.4.1 单副本并发扫描:拐点在 32,但可卖并发只有 8 `[实测 2026-09-15]`
...
> ⚠️ 这一轮是 **0% 命中**(每个 prompt 内容唯一)。现网 78% 命中时 prefill 工作量
> 只有 22%,真实可卖容量会高得多。**热态扫描还没做,这是本节最大的缺口。**
...
1. **热态并发扫描**(§3.4.1 的缺口)。现有拐点曲线是 0% 命中下测的,
   **现网 78% 命中时的可卖容量仍然未知** —— 而这正是定价和限流阈值要用的数。
   做法:共享前缀的闭环压测,重跑 §3.4.1 那条曲线。**这是最便宜的高价值项。**
