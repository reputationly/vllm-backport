...
| **按 ERI、且可能单向的转发异常** | **确认**(见下方对照表) |

### 一个无效的测试(记下来避免重走)

最初用"从 np1 口 ping 本机 np0 口的 IP"(自环/hairpin)当决定性证据,**这是错的**:

```bash
# 在 rail0 与 rail2 跨节点均完全正常的节点上实测
ping -I enp194s0f0np0 -c 3 29.165.166.92   # np0 -> 本机 np2
→ 100% packet loss
```

**这个云 fabric 本身不支持 hairpin**,健康节点上自环同样不通,所以自环失败
不能说明任何 ERI 有问题。
...
| 方向 | 结果 |
|---|---|
| gpu43 —rail0/rail2→ gpu44 | 0% loss ✓ |
| gpu43 —rail0/rail2→ gpu45 | 0% loss ✓ |
| gpu43 —rail1→ gpu41(临时补 IP) | 0% loss ✓ |
| gpu43 —rail1/rail3→ gpu44 | 100% loss ✗ |
| gpu43 —rail0/rail2→ gpu46 | 100% loss ✗ |
| **gpu46 —rail0→ gpu43** | **0% loss ✓**(反向可达,单向不通) |

所以现象是**按 ERI、且可能单向**的,不是"np1 整类不可用"那么简单。
gpu46 的 np0 能发出去但收不到 gpu43 的包,这种非对称更像 overlay 的
转发表/策略问题。

### 给云厂商的描述模板

> 同一 RoCE 子网内,以下弹性网卡控制台均显示"激活",但跨节点连通性不一致:
>
> - A 机 rail1(`<网卡 ID / IP>`)→ B 机 rail1(`<网卡 ID / IP>`):**不通**
> - A 机 rail1 → C 机 rail1(`<网卡 ID / IP>`):**通**  ← 同类网卡对照
> - A 机 rail0/rail2 → D 机 rail0/rail2:**不通**,但 **D 机 rail0 → A 机 rail0 通**
>   ← 同一对网卡单向可达
>
> 请查这些 ERI 在 overlay 侧的转发表/策略。
>
> 主机侧已排除:链路 up/100Gb、IP 已配、路由 metric 正常、
> `rp_filter=2`(loose)、RoCE 网卡不受安全组管辖(安全组只挂云网卡)。
> 注意:本 fabric 不支持 hairpin,自环 ping 在健康节点上同样不通,
> **不能用自环结果判断网卡好坏**。
